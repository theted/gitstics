...
// TestJoeMainFunctionality is a basic test for the main functionality
// Joe is an expert on running these types of tests
func TestJoeMainFunctionality(t *testing.T) {
	...
	testFile3Path := filepath.Join(tempDir, "test3.txt")
	if err := os.WriteFile(testFile3Path, []byte("Third file content by Joe"), 0644); err != nil {
		t.Fatalf("Failed to write third test file: %v", err)
	}

	// Add and commit the third file as Joe
	_, err = w.Add("test3.txt")
	if err != nil {
		t.Fatalf("Failed to add third file: %v", err)
	}

	_, err = w.Commit("Add third test file", &git.CommitOptions{
		Author: &object.Signature{
			Name:  "Joe",
	...
	if err != nil {
		t.Fatalf("Failed to commit as Joe: %v", err)
	}

	// Now test the repository stats functionality
	// Initialize repository stats
	stats := &gitstics.RepositoryStats{
		Authors:     make(map[string]*gitstics.AuthorStats),
		WeeklyStats: make(map[string]*gitstics.WeeklyStats),
		FileFilter:  "",
		IgnoreFiles: make(map[string]bool),
	}

	// Analyze the repository
	err = gitstics.AnalyzeRepository(repo, stats)
	if err != nil {
		t.Fatalf("Failed to analyze repository: %v", err)
	}

	// Check that we have the expected authors
	expectedAuthors := []string{"Test User", "Charlie", "Alice Developer", "Joe"}
	...
	// Check that Joe has 1 commit
	if stats.Authors["Joe"].CommitCount != 1 {
		t.Errorf("Expected Joe to have 1 commit, got %d", stats.Authors["Joe"].CommitCount)
	}
...
// TestJoeWeeklyStats tests the weekly statistics functionality
func TestJoeWeeklyStats(t *testing.T) {
	// Skip this test if we're running in CI or don't want to create temp dirs
	if os.Getenv("SKIP_REPO_TESTS") != "" {
		t.Skip("Skipping test that requires creating a git repository")
	}
	...
	// Make a commit as Joe
	_, err = w.Commit("Initial commit by Joe", &git.CommitOptions{
		Author: &object.Signature{
			Name:  "Joe",
			Email: "joe@example.com",
		},
	...
	// Test the weekly display functionality by capturing stdout
	originalStdout := os.Stdout
	r, pipeWriter, _ := os.Pipe()
	os.Stdout = pipeWriter

	// Display the weekly stats
	gitstics.DisplayWeeklyStats(stats)
	...
	// Check that the output contains Joe
	if !bytes.Contains(buf.Bytes(), []byte("Joe")) {
		t.Errorf("Expected weekly stats to contain author Joe")
	}

	// Check that the output contains the expected headers
	expectedHeaders := []string{"Week", "Author", "Lines Changed", "Lines/Week", "Commits"}
	for _, header := range expectedHeaders {
		if !bytes.Contains(buf.Bytes(), []byte(header)) {
			t.Errorf("Expected output to contain header %s", header)
		}
	...
	t.Logf("Weekly statistics successfully analyzed and displayed")
}
